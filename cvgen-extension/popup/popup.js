...
function showScreen(name) {
  ...
  document.getElementById('screen-connected').style.display = name === 'connected' ? 'flex' : 'none';
}
...
function updateSyncBadge(isoDate) {
  ...
  if (diffH < 1) {
    ...
    relativeLabel = d.toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' });
  }

  label.textContent = 'Profil synchronisé — ' + relativeLabel;
  dot.className = diffH > 48 ? 'sync-dot sync-dot--warning' : 'sync-dot sync-dot--ok';
}
...
async function updateSiteIndicator() {
  ...
  if (config) {
    ...
    siteNameEl.innerHTML = config.name + ' <span style="color:#22c55e">' + config.icon + ' détecté</span>';
  } else {
...
async function refreshDetectedFields() {
  ...
  try {
    ...
    document.getElementById('site-indicator').querySelector('#site-name').textContent =
      'Naviguez sur un formulaire de candidature';
  }
}

const FIELD_TYPE_LABELS = {
  prenom: 'Prénom', nom: 'Nom', nom_complet: 'Nom complet',
  email: 'Email', telephone: 'Téléphone',
  adresse: 'Adresse', ville: 'Ville', code_postal: 'Code postal',
  pays: 'Pays', region: 'Région',
  titre_poste: 'Titre du poste', resume_professionnel: 'Résumé',
  annees_experience: 'Années d\'expérience', niveau_etudes: 'Niveau d\'études',
  lettre_motivation: 'Lettre de motivation', disponibilite: 'Disponibilité',
  salaire: 'Prétention salariale', type_contrat: 'Type de contrat',
  linkedin: 'LinkedIn', portfolio: 'Portfolio', github: 'GitHub',
  genre: 'Civilité', handicap: 'RQTH'
};
...
document.getElementById('login-form').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    if (response && response.success) {
      ...
      errorEl.textContent   = response && response.error
        ? response.error
...
document.getElementById('link-generate-cover').addEventListener('click', function (e) {
  e.preventDefault();
...
document.getElementById('btn-fill').addEventListener('click', async function () {
  ...
  try {
    const response = await sendToContentScript({ type: 'FILL_FORM', options: { overwrite: false } });

    if (response && response.success) {
      const n = response.filled;
      const t = response.total;
      resultEl.textContent = n + ' champ' + (n > 1 ? 's' : '') + ' rempli' + (n > 1 ? 's' : '') +
        ' sur ' + t + ' détecté' + (t > 1 ? 's' : '');
      resultEl.className = 'fill-result success';
    } else {
      resultEl.textContent = response && response.error ? response.error : 'Erreur lors du remplissage.';
...
async function sendToContentScript(message) {
  ...
  try {
    ...
    if (!firstErr.message.includes('Receiving end') && !firstErr.message.includes('Could not establish')) {
      throw firstErr;
    }
  }
  ...
  try {
    ...
    throw new Error('Impossible d\'injecter le script sur cette page. Rechargez la page (F5) puis réessayez.');
  }
...
async function getActiveTab() {
  return new Promise(function (resolve) {
    ...
    chrome.tabs.query({ active: true, lastFocusedWindow: true }, function (tabs) {
      if (tabs && tabs[0]) { resolve(tabs[0]); return; }
